...
def add_planet():
    ...
    new_planet = Planet(name = body_name, population = body_population, climate = body_climate, terrain = body_terrain)
...
def add_character():
    ...
    try:
        db.session.commit() # commit ->
        return jsonify({"msg": "Successfully created character!"}), 201
...
def deletePlanetOfFavorites(user_id,planet_id):
    ...
    if favoritePlanet is None:
        return jsonify({"mensaje": "este planeta no esta en favoritos"}), 400
    else:
        try:
            db.session.delete(favoritePlanet)
            db.session.commit()
            return jsonify({"mensaje": "planeta  favorito eliminado con exito!"}), 200
        except Exception as error:
            db.session.rollback()
...
def addCharacterToFavorites(user_id,character_id):
    ...
    if character is None:
        return jsonify({"mensaje": "este personaje no existe"}), 400
    ...
    if characterFavorites is not None:
        return jsonify({"mensaje": "este personaje ya esta en favoritos"}), 400
    else:
        try:
            favorite = Favorite(user_id = user_id , name = character.name, character_id = character_id)
            db.session.add(favorite)
            db.session.commit()
            return jsonify({"mensaje": "personaje favorito agregado con exito!"}), 200
        except Exception as error:
            db.session.rollback()
            return {"error": error}, 500
...
def deleteCharacterOfFavorites(user_id,character_id):
    ...
    if favoriteCharacter is None:
        return jsonify({"mensaje": "este personaje no esta en favoritos"}), 400
    else:
        try:
            db.session.delete(favoriteCharacter)
            db.session.commit()
            return jsonify({"mensaje": "personaje favorito eliminado con exito!"}), 200
        except Exception as error:
            db.session.rollback()
            return {"error": error.args[0]}, 500
    return
